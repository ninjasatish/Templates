...
template <class T, size_t siz> void _print(array<T, siz> array){cerr << "{ "; for(ll i = 0; i < siz; ++i){_print(array[i]); cerr<<' ';} cerr<<"}";}
...
int main (){
   ...
   cerr << "Time = " << chrono::duration_cast<chrono::microseconds>(end - begin).count() / 1000 << " ms" << nline;
}